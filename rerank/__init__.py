...
class ContextualAIReranker(pt.Transformer):
    def __init__(
            self,
        ...
        """
        Parameters:
          model: The identifier of the Contextual AI model to use.
          instruction: Additional instruction text to guide reranking.
          text_key: The DataFrame column that contains the candidate document text.
          metadata_key: The DataFrame column that contains document metadata (if available). If not present,
                        an empty string is used for each document.
        The API token and (optionally) base URL are read from the .env.local file.
        Expected variables:
          CONTEXTUAL_AI_API_TOKEN
          CONTEXTUAL_AI_BASE_URL (optional; default is "https://api.contextual.ai/v1")
        """
        self.api_key = os.getenv("CONTEXTUAL_AI_API_TOKEN")
        if not self.api_key:
            raise ValueError("CONTEXTUAL_AI_API_TOKEN not set in .env.local")
        self.base_url = os.getenv("CONTEXTUAL_AI_BASE_URL", "https://api.contextual.ai/v1")
        self.model = model
    ...
    def transform(self, retrieved: pd.DataFrame) -> pd.DataFrame:
        """
        Reranks the candidates in the DataFrame using the Contextual AI API.

        Assumptions:
          - The DataFrame has a 'query' column (same for all rows).
          - Each candidate row has a unique document id in 'docno'.
          - The document text is stored in the column defined by `self.text_key`.
          - Optionally, if the DataFrame contains a column specified by `self.metadata_key`,
            it will be used as metadata for each document; otherwise an empty string is used.

        The API response should return a JSON object with either a "scores" field (a list of scores
        corresponding to the input order) or a "ranking" field. The DataFrame is updated with new 'score'
        and 'rank' columns.
        """
        # Create a copy to avoid modifying the original DataFrame
        retrieved = retrieved.copy()
        # Get the query (assumed the same for all candidates)
        query = retrieved.iloc[0].query
        # Extract document texts
        documents = retrieved[self.text_key].tolist()
        # Use metadata if available; otherwise use empty strings
        if self.metadata_key in retrieved.columns:
            metadata = retrieved[self.metadata_key].tolist()
        else:
            metadata = ["" for _ in range(len(retrieved))]
        # Build the payload for the Contextual AI API
        payload = {
            "query": query,
        ...
        # Construct the API endpoint URL and send the POST request
        rerank_api_endpoint = f"{self.base_url}/rerank"
        response = requests.post(rerank_api_endpoint, json=payload, headers=headers)
        if response.status_code != 200:
            raise Exception(f"Contextual AI API error: {response.status_code} {response.text}")
        result = response.json()
        # Try to extract scores directly. If not present, see if a ranking is provided.
        scores = result.get("scores")
        if scores is None:
            ranking = result.get("ranking")
            if ranking is None:
                raise Exception("Invalid response from Contextual AI API: missing both 'scores' and 'ranking'.")
            # If only ranking is returned, we compute reciprocal ranking scores.
            scores = [1 / (rank + 1) for rank in ranking]
        # Update the DataFrame with the new scores and compute the ranking order.
        retrieved['score'] = scores
        retrieved.sort_values(by='score', ascending=False, inplace=True)
        retrieved['rank'] = range(len(retrieved))
        return retrieved

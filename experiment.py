...
def run_experiment(EVALUATION_NAME, index_name, field):
    ...
    result_dfs = []
    result = pt.Experiment([bm25], queries, dataset.get_qrels(), EVAL_METRICS, ['BM25'], round=3, verbose=True,
                           batch_size=10)
...
def run_experiment_rankllm(llm_reranker, EVALUATION_NAME, index_name, field, exp_name):
    ...
    result_dfs = []
    result = pt.Experiment([bm25, reranking_pipe], queries, dataset.get_qrels(), EVAL_METRICS,
                           ['BM25', 'BM25>>GenRank'],
...
if __name__ == '__main__':
    IR_DATASETS = [['irds:msmarco-passage/trec-dl-2019/judged', 'msmarco_passage', 'text', 'text']]
    triplet = IR_DATASETS[0]
    EVALUATION_NAME = triplet[0];
    index_name = triplet[1];
    field = triplet[2];
    doc_field = triplet[3]
    # m_reranker = LLMReRanker("castorini/rank_vicuna_7b_v1")
    # n_experiment_rankllm(llm_reranker, EVALUATION_NAME, index_name, field, 'rank_vicuna_7b_v1')
    # llm_reranker = LLMReRanker("meta-llama/Meta-Llama-3-70B-Instruct")
    # run_experiment_rankllm(llm_reranker, EVALUATION_NAME, index_name, field, 'meta-llama/Meta-Llama-3-70B-Instruct')
    llm_reranker = LLMReRanker("meta-llama/Meta-Llama-3-8B-Instruct")
    run_experiment_rankllm(llm_reranker, EVALUATION_NAME, index_name, field, 'Meta-Llama-3-8B-Instruct')
    llm_reranker = LLMReRanker("castorini/LiT5-Distill-xl")
    run_experiment_rankllm(llm_reranker, EVALUATION_NAME, index_name, field, 'LiT5-Distill-xl')
